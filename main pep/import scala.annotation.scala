...
// Implement below the shunting yard algorithm. The most
...
@tailrec
def syard2(toks: Toks, st: Toks = Nil, out: Toks = Nil) : Toks = /*toks.head match*/ {
if(toks != Nil) toks.head match{

case "*" => if (grab ("*", st) != Nil) syard(toks.tail, List("*") :::st.takeRight (st.length - grab ("*", st).length), out ::: grab ("*", st) )
...
// Implement a compute function that evaluates an input list
