...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    val list = lines.filter(_.split(",")(2).toDouble>=4)
    val result = list.map(_.split(",")).map(rating=>(rating(0).trim, rating(1).trim))
    result
}

def process_movies(lines: List[String]) : List[(String, String)] ={
    val result = lines.map(_.split(",")).map(film=>(film(0).trim,film(1).trim))
    result
}
...
def groupById(ratings: List[(String, String)], 
              m: Map[String, List[String]]) : Map[String, List[String]] ={
                  if (ratings.size <= 0) m 
                      ...
                      val sort = ratings.filter(_._1 == ratings.head._1).map(a=>a._2)
                      val sort2 = m + (ratings.head._1 -> sort)
                      groupById(ratings.filter(_._1 != ratings.head._1), sort2)
                  }
              }
...
def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
    m.filter(x => x._2.contains(mov)).map(x=>x._2.filterNot(_==mov)).toList
}
...
def suggestions(recs: Map[String, List[String]], 
                mov_name: String) : List[String] = {
                    ...
                    val favMov= favourites(recs,mov_name).flatten.groupBy(identity)
                    val favMovList= favMov.toList.sortBy(_._2.size)
                    val result= favMovList.reverse.map(x =>x._1)
                    result
                }
...
def recommendations(recs: Map[String, List[String]],
                    ...
                    mov_name: String) : List[String] = {
                        ...
                        val result = if(sug.size == 1) List(movs(sug(0)))
                        else if (sug.size < 1) Nil
                        else  {List(movs(sug(0)),movs(sug(1)))}
                        result
                    }
...
def most_recommended(recs: Map[String, List[String]],
                    ...
                    //all.flatten.distinct.map(nn => nn -> all.count(_.contains(nn))).toMap.toSeq.sortWith(_._2 > _._2).toList
