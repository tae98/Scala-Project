...
def clean(s: String) : List[String] = {
    //regex finds single words 
    val foo = """\w+""".r
    // put them in to the list
    foo.findAllIn(s).toList
}
...
def occurrences(xs: List[String]): Map[String, Int] = {
    //finds existing strings
    val uniqueString = xs.distinct
    //pair string with number of appearance
    val numOccur= uniqueString.map(x=>(x, xs.count(_==x)))
    numOccur.toMap
}
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    //change map to list
    val occurLst1 = occurrences(lst1).toList
    val occurLst2 = occurrences(lst2).toList
    //gives values to the strings by its repetition
    def stringValues(xi: (String, Int), lst: List[(String,Int)]): Int={
        val target = xi._1
        if (lst.find(_._1 == target)!=None){lst.find(_._1 == target).toList(0)._2}
        else 0
    }
    //multiply the values
  val result =  occurLst1.map(wrd => stringValues(wrd,occurLst1) * stringValues(wrd,occurLst2))
  //add the values
  result.sum
   }
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
  val d1d2= prod(lst1,lst2).toDouble

  val dd1 = prod(lst1,lst1)
  val dd2 = prod(lst2,lst2)

  val max = List(dd1,dd2).max
  val result= d1d2 / max
  result
}
